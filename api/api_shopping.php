...
try {
    ...
    foreach ($itemRows as $i => $itemRow) {
        $sql = "select i.no, i.name, i.price, i.img, i.amount from `item` i join `seller` s on (i.seller_no = s.no) join `member` m on (s.member_no = m.no) where s.no = {$itemRow['no']} and i.no in ($itemArr)";
